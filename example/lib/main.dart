...
class _HomeState extends State<Home> {
  final sdk = AadhaarAuthSdk.instance;
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  final res = await sdk.integrate(context, appCode:appCode, userData:userData);

                  if(res!=null && res.status.isNotEmpty){
                    setState(() => status = res.message ?? 'null');
                  }else{
                    setState(() => status = res?.message ?? 'null');
                  }
                } catch (e) {
